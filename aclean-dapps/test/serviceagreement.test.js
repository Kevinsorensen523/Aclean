...
describe("ServiceAgreement", async () => {
  ...
  it("agreement status can only be updated by provider", async () => {
    try {
      ...
      expect(err.message.match(/Only the service provider can call this/)).to.be
        .ok;
    }
  });
  ...
  it("can get agreement balance", async () => {
    expect(await instance.getAgreementBalance()).to.equal(BigNumber.from(0));

    await instance.connect(client).deposit({ value: cost });
    expect(await instance.getAgreementBalance()).to.equal(BigNumber.from(cost));
  });
  ...
  it("won't allow a transfer of funds to the provider without the correct smart contract balance", async () => {
    try {
      ...
      expect(err.message.match(/Contract balance requirement not met/)).to.be
        .ok;
    }
  });
  ...
  it("won't allow a transfer of funds to the provider without service completed", async () => {
    ...
    try {
      ...
      expect(err.message.match(/Service was not completed or approved/)).to.be
        .ok;
    }
  });
